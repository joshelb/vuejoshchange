...
export default {
  ...
  setup() {
    return {
      columnDefs: [
				{ field: "index", cellRenderer: "btnCellRenderer", cellRendererParams: { 
          clicked(field) {
	...
	methods: {
      ...
      this.gridColumnApi = params.columnApi;
        ...
        for (let i = 0; i < data.length; i++) {
          ...
          var price = data[i][4];
          var side = data[i][2];
          var row = {index: index, orderid: orderid, timestamp: timestamp ,quantity: quantity,price: price, side: side};
          grid.push(row);
        }
