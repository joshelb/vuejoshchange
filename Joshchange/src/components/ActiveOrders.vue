...
export default {
  ...
  setup() {
    return {
      ...
      width: '100%',
      columnDefs: [
				{ field: "index", headerName: '',cellRenderer: "btnCellRenderer", cellRendererParams: { 
          clicked(field) {
	...
	methods: {
      ...
      this.gridColumnApi.setColumnVisible('timestamp', false)
        ...
        for (let i = 0; i < data.length; i++) {
          ...
          var price = data[i][4];
          var side = data[i][2];
          var row = {index: index, orderid: orderid, timestamp: timestamp ,quantity: parseFloat(parseFloat(quantity).toFixed(10)),price: parseFloat(parseFloat(price).toFixed(10)), side: side};
          grid.push(row);
        }
